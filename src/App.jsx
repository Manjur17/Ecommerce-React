...
function App() {
  ...
  async function load() {
    ...
    //on initial load of app after the user login and user state is set then fetch user cart  
    if(user) {
  ...
  useEffect(() => {
    load(); //it is caed twice when user is not set(just logs in on initial load) and when user is set. 
  }, [user]);
